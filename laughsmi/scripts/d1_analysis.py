"""D1.6 — Analyze the effect of splicing synthetic laughter into fake audio.

Pairs base vs. aug spoof scores by file_id (basename), restricted to files the
manifest marks augmented==1, and reports:
  - paired Delta score (aug - base): mean/median, Wilcoxon signed-rank, effect size
  - evasion: fraction of augmented fakes whose score drops below the clean-EER
    threshold after laughter insertion (i.e. now classified bona-fide)
  - split of Delta by insert position (start/mid/end)
  - clean vs aug spoof-detection at the clean-EER operating threshold
Writes tables/table_d1.csv and .tex.
"""
...
def analyze(name, base_csv, aug_csv, manifest_csv):
    ...
    # evasion: fake now scored below threshold (looks bona-fide)
    evaded_after = (aug_s < thr)
    caught_before = (base_s >= thr)
    newly_evaded = int(np.sum(caught_before & (aug_s < thr)))
...
def main():
    ...
    ap.add_argument("--out", default=str(LAUGHSMI / "tables" / "table_d1.csv"))
    args = ap.parse_args()
    do = LAUGHSMI / "detector_out"; da = LAUGHSMI / "data"
    results = []
    results.append(analyze("ASVspoof19 (asv19 model)", do/"base_asv19.csv", do/"aug_asv19.csv", da/"eval_asv19_aug/manifest.csv"))
    results.append(analyze("MLAAD (mlaad model)", do/"base_mlaad.csv", do/"aug_mlaad.csv", da/"eval_mlaad_aug/manifest.csv"))
