#!/usr/bin/env python3
"""
Train the regular (mlaad_goat) model on MLAAD-tiny.

Converted from mixed.ipynb. Training logic preserved exactly; changes:
...
Usage:
    python experiments/scripts/train_mlaad_regular.py

Produces:
    experiments/checkpoints/mlaad_goat.ckpt
    experiments/results/mlaad/training_logs/mlaad_goat/
        training_curves.csv
        final_metrics.json
...
HP = {
    ...
    "batch_size": 20,
    "max_epochs": 7,
    "reverb_prob": 0.30,          # 30% reverb  (matches mixed.ipynb)
...
# Deviations from the ASVspoof runs:
DEVIATIONS = [
    "No random crop: preprocessed .pt files are center-cropped to 48000 samples "
    "(ASVspoof used random crop during training). Center crop used for all splits.",
    "No train-sample limit: ASVspoof used limit=15000; MLAAD-tiny has fewer samples, "
    "so all available balanced pairs are used.",
    "No wandb logging: replaced by CSV + JSON for reproducibility / no credential dependency.",
...
def apply_reverb(wav: torch.Tensor, t60: float, room_scale: float = 0.5) -> torch.Tensor:
    T = wav.shape[-1]
...
class MAALDSplitDataset(Dataset):
    ...
    Deviation from ASVspoof: audio is center-cropped, not randomly cropped.
    """
    ...
    def __init__(
        ...
        reverb_t60_values: list[float] | None = None,
    ):
        self.processed_dir = processed_dir
        self.mode = mode
        self.reverb_prob = reverb_prob
        self.reverb_t60_values = reverb_t60_values or [0.20, 0.35, 0.50, 0.70, 0.90]
        ...
        if n_bonafide == 0 or n_spoof == 0:
            raise RuntimeError(
                ...
                "Both classes are required. Re-run prepare_mlaad_tiny.py after "
                "bonafide data is downloaded."
            )
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.mode == "train" and random.random() < self.reverb_prob:
            t60 = random.choice(self.reverb_t60_values)
            wav = apply_reverb(wav, t60)
...
class CSVTrainingLogger:
    """Accumulates per-epoch metrics and writes training_curves.csv."""
    ...
    def log_epoch(self, epoch: int, metrics: dict) -> None:
        row = {
            ...
            "val_eer": metrics.get("val-eer", float("nan")),
            "val_acc": metrics.get("val-acc", float("nan")),
            "val_auc": metrics.get("val-auc", float("nan")),
        }
        self.rows.append(row)
        with open(self.csv_path, "a") as f:
            vals = [str(row[c]) for c in self.COLUMNS]
            f.write(",".join(vals) + "\n")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=PROJECT_ROOT / "experiments/checkpoints/mlaad_goat.ckpt",
    )
    p.add_argument(
        "--log-dir", type=Path,
        default=PROJECT_ROOT / "experiments/results/mlaad/training_logs/mlaad_goat",
    )
...
def main() -> None:
    ...
    hp_snapshot.update({
        ...
        "script": "train_mlaad_regular.py",
        "model_variant": "mlaad_goat",
    ...
    train_ds = MAALDSplitDataset(
        ...
        balance=True,
        seed=args.seed,
        reverb_prob=HP["reverb_prob"],
        reverb_t60_values=HP["reverb_t60_values"],
    )
    ...
    class MetricsCollectorCallback:
        """Captures epoch-end metrics from trainer for CSV logging."""
        pass
    ...
    trainer = Trainer(
        ...
        deterministic=False,  # deterministic=True slows WavLM significantly
    ...
    # ── Write training_curves.csv from PL CSV logs ────────────────────────────
    ...
    if pl_metrics_path.exists():
        ...
        # Aggregate per epoch (PL logs step-level, aggregate to epoch)
    ...
    # ── Final metrics ─────────────────────────────────────────────────────────
    # Find best checkpoint val-eer — check callback first, then scan ckpt dir
    import re as _re
    ...
    try:
        ...
        if not best_model_path:
            # Fallback: scan checkpoint dir for best-epoch file
    ...
    # Also get last epoch metrics from logged_metrics
    last_metrics = {}
    try:
